 ...
 *  Tweaked for the Zynq bootrom (128K in size) 5-2-2024
 *  Tweaked for STM32H743 demo code (192K) 4-29-2025
 */
#include <stdio.h>
#include <unistd.h>
#include <stdlib.h>
...
#ifdef notdef
/* Zynq */
unsigned int rombase = 0x00000000;
unsigned int entry = 0x00000000;
#endif

/* STM32H743 */
unsigned int rombase = 0x08000000;
unsigned int entry   = 0x08000000;
...
int main ( int argc, char **argv )
{
	...
	// This is NOT the magic to get Thumb disassembly
	// hdr.e_flags = swap_long ( 0x05000200 );
	...
	printf ( "Write pad  %d bytes\n", npad );
	write ( of, pad, npad );		/* Write pad to page boundary */

	printf ( "Write ROM image  %d bytes\n", romsize );
	/* The ROM image !!! */
	write ( of, buf, romsize );		/* Write ROM image */

	printf ( "Write Gnu signature %d bytes\n", gnusize );
	/* The gnu signature */
...
mk_phdr ( int of )
{
	...
	printf ( "Write program header  %d bytes\n", sizeof(phdr) );
